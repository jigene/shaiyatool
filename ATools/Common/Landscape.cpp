...
bool CLandscape::Load(int x, int y, int textureID)
{
	//CFile file;
...
void CLandscape::SetupHeightMap(std::vector<float> mapData, int sideLen, int x, int y, int length) {
	...
	for (int i = 0; i < (MAP_SIZE + 1) * (MAP_SIZE + 1) && i < length; i++) {
		...
		m_heightMap[i] = mapData[((y + rowOffset) / 2 * sideLen * 2 + (x + posOffset++) / 2 * 2)];

		if (posOffset > MAP_SIZE) {
		
			rowOffset++;
			posOffset = 0;

			if (rowOffset > MAP_SIZE) {
