...
void CProject::LoadGround(string textureFile, int index)
{
	m_terrains[index] = textureFile.replace(".tga", ".dds");
}
	...
	if (reader.is_open()) {
		...
		for (int counter = 0; counter < total; counter++) {
			...
			if (*(filedata + counter) == '\0') {
				...
				if (found) {
					...
					m_terrains[10 + (waterCount++)] = string(filetext.c_str()).replace(".jpg", ".dds");
					found = false;
				}
		...
		if (waterCount > 0) {
			...
			for (int counter = 0; counter < waterCount; counter++) {
				...
				m_waterLists[counter].name = m_terrains[100 + counter];
				m_waterLists[counter].textureCount = 1;
				m_waterLists[counter].textures = new CTexture*[waterCount];
				m_waterLists[counter].textures[0] = TextureMng->GetTerrainTexture(m_terrains[100 + counter]);
				m_waterLists[counter].textureIDs = new int[1]{100 + counter};
			}
