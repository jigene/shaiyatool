		...
		for (int counter = current; counter < total; counter++) {
			...
			if (counter == 5) {
				...
				if (retLongVal == 2048) {
					...
					qCritical("Light and height map information has not been setup for 1240 maps. Results will be incorrect.");
					heightMapEnd = 789515;
				}

				current = heightMapEnd + 1;

				break;
			}
			else if (lightMapNotSend && lightMapStart > 0 && lightMapEnd > 0) {

				lightMapNotSend = false;
				m_tokenType = ETokenType::Number;
				retIntVal = lightMapEnd - lightMapStart;
				memcpy(m_token, string(std::string("lightmap").c_str()).constData(), 8 * sizeof(QChar));
				filetext = "";
				break;
			}
			...
			else if (filetext.find(".tga") != std::string::npos && skyDataFound) {
